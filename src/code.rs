...
impl Adapter {
    ...
    fn attached_symbol<'a>(
        ...
        let mut attached = symbols
            .iter()
            .filter(|s| {
                s.attach_start >= comment.end_byte()
                    && source[comment.end_byte()..s.attach_start].trim().is_empty()
            })
            .collect::<Vec<_>>();
        attached.sort_by_key(|s| s.attach_start);
        if let Some(first) = attached.first() {
            if attached
                .get(1)
                .is_some_and(|next| next.attach_start == first.attach_start)
            {
                return Err(());
            }
...
fn collect<'a>(
    node: Node<'a>,
    declarations: &BTreeMap<usize, (String, Node<'a>, bool)>,
    separator: &str,
    path: &Path,
    lines: &[usize],
    ...
    let declaration = declarations.get(&node.id());
    if let Some((name, name_node, selectable)) = &declaration {
        ...
        if *selectable {
            ...
            symbols.push(CodeSymbol {
                ...
                source: location(path, lines, name_node.start_byte(), name_node.end_byte()),
                content: location(path, lines, node.start_byte(), node.end_byte()),
    ...
    for child in node.named_children(&mut cursor) {
        collect(child, declarations, separator, path, lines, prefix, symbols);
    }
