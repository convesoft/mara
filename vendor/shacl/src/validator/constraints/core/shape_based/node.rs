    ...
    fn validate(
        ...
        for (fnode, nodes) in value_nodes.iter() {
            ...
            for node in nodes.iter() {
                ...
                let had_violations = if !crate::validator::bounded::active() && engine.has_validated(&node_object, *shape_idx) {
                    engine
                    ...
                    crate::validator::bounded::explain(&shape.id().to_string(), &fnode.to_string(), &component_obj.to_string(), &inner_results);
