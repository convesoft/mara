...
impl<RDF: NeighsRDF + Debug> Validate<RDF> for IRShape {
    fn validate(
        ...
        for node in focus_nodes.iter() {
            crate::validator::bounded::visit(&node.to_string())?;
        }
        ...
        let uncached_focus_nodes = if let Some(idx) = idx {
            ...
            for fnode in focus_nodes.iter() {
                ...
                if let Some(outcome) = runner.get_cached_outcome(&obj, *idx).filter(|_| !crate::validator::bounded::active()) {
        ...
        // Interleave local components and property obligations in authored source
        // order supplied by the host. RDF/hash iteration order is not observable.
        let mut checks = self.components().iter().map(|c| (crate::validator::bounded::order(
            &self.id().to_string(), &Object::iri(c.into()).to_string()), Some(c), None)).collect::<Vec<_>>();
        for ps in self.property_shapes() {
            let shape = shapes_graph.get_shape_from_idx_e(ps)?;
            checks.push((crate::validator::bounded::order(&shape.id().to_string(), ""), None, Some(shape)));
        }
        checks.sort_by_key(|(order, component, property)| (*order, component.map(ToString::to_string), property.map(|s| s.id().to_string())));
        let mut component_outcome = ValidationOutcome::new();
        let mut nested_by_focus: HashMap<Object, ValidationOutcome> = HashMap::new();
        for (_, component, property) in checks {
            if let Some(component) = component {
                crate::validator::bounded::component(&self.id().to_string(), component, &value_nodes)?;
                let outcome = runner.evaluate(store, self, component, &value_nodes, source_shape, self.path(), shapes_graph)?;
                crate::validator::bounded::record(&outcome);
                component_outcome.extend(outcome);
            } else if let Some(shape) = property {
